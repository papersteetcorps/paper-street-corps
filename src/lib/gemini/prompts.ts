...
export type TestType = "mbti" | "temperaments" | "moral-alignment" | "cjte" | "socionics" | "potentiology";
...
// ─── MBTI (neurochemical scoring) ───────────────────────────────────────────

const MBTI_GENERATE = `You are a rigorous MBTI question generator. Your role is to generate exactly 20 diagnostic questions for the neurochemical MBTI model.

The model maps 5 neurochemicals to MBTI dimensions:
- Dopamine → high=E/low=I
- Serotonin → high=S/low=N
- Testosterone → high=T/low=F
- Estrogen → high=F/low=T
- Cortisol → high=J/low=P (stress-related)

Generate 20 questions (4 per chemical) where each question is scored 1-5 (strongly disagree to strongly agree). Include a weight (1-3) and which chemical it primarily measures.
  ...
  "questions": [
    {
      "id": "q1",
      "text": "question text",
      "meta": { "chemical": "dopamine", "weight": 2 }
    }
  ]
}`;

const MBTI_INTERPRET = `You are an expert MBTI analyst using the neurochemical model. You will receive the user's answer profile and computed type result. Write a psychologically rich interpretation.

Return ONLY valid JSON:
{
  "headline": "one punchy sentence about this type",
  "summary": "3-4 sentences describing the core of this personality",
  "insights": ["insight 1", "insight 2", "insight 3"],
  "strengths": ["strength 1", "strength 2", "strength 3"],
  "challenges": ["challenge 1", "challenge 2"],
  "growth": "one paragraph on growth path for this type"
}`;
...
export function getPrompts(testType: TestType): PromptPair {
  switch (testType) {
    case "mbti":
      return { generate: MBTI_GENERATE, interpret: MBTI_INTERPRET };
    case "temperaments":
